...
function TopicPieChart({ data, colorScale, size, hoveredTopic }: TopicPieChartProps) {
    ...
    useEffect(() => {
        ...
        const g = svg.selectAll("g").data([null]).join("g").attr("transform", `translate(${size / 2}, ${size / 2})`);

        const pie = d3.pie<TopicStats>()
            .value(d => d.total)
            .sort(null);

        const arcGenerator = d3.arc<d3.PieArcDatum<TopicStats>>()
    ...
    return (
        <Box position="relative" w={size} h={size}>
            ...
            <Center position="absolute" top={0} left={0} right={0} bottom={0} pointerEvents="none">
                <VStack spacing={0}>
                    <Text fontSize="3xl" fontWeight="bold" color="whiteAlpha.900" lineHeight="1">
                        {total}
                    </Text>
                    <Text fontSize="xs" color="gray.400">Total Solved</Text>
                </VStack>
            </Center>
        </Box>
    );
        ...
        <VStack spacing={1} w="100%" align="stretch">
            {data.map(topic => (
                ...
                >
                    ...
                    <Text fontSize="sm" color="whiteAlpha.800" noOfLines={1}>{topic.domain}</Text>
                    <Spacer />
                    <Text fontSize="sm" fontWeight="bold" color="whiteAlpha.900">{topic.total}</Text>
                </HStack>
            ))}
        </VStack>
    )
...
export default function AnalyticsSection() {
  const [analyticsData, setAnalyticsData] = useState<{topics: TopicStats[], difficulties: DifficultyCounts} | null>(null);
  ...
  const topicColorScale = useMemo(() => {
      // ✅ FIX: Add a guard clause to ensure the theme object is loaded.
      if (!theme || !theme.colors) {
        return d3.scaleOrdinal<string>(); // Return a default/empty scale if theme is not ready
      }
  ...
  const topTopics = useMemo(() => analyticsData?.topics.slice(0, 15) || [], [analyticsData]);
    ...
    >
        ...
        {isLoading ? (
            ...
            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={6} w="100%" alignItems="center">
                {/* Left Section: Chart */}
                <Center>
                    <TopicPieChart data={topTopics} colorScale={topicColorScale} size={220} hoveredTopic={hoveredTopic} />
                </Center>
                ...
                <VStack align="stretch" spacing={3}>
                    ...
                    <Box 
                        maxHeight="240px" // Set a max height for the legend
                    ...
                    >
                        <TopicLegend data={topTopics} colorScale={topicColorScale} onHover={setHoveredTopic} />
                    </Box>
                    <HStack spacing={3} color="gray.400" justify="center" w="100%" wrap="wrap" pt={2}>
                        <HStack align="center" spacing={1.5}>
                            <Box w="8px" h="8px" bg="green.400" borderRadius="full" />
                            <Text fontSize="xs">Easy: <chakra.span fontWeight="bold" color="whiteAlpha.800">{analyticsData.difficulties.easy}</chakra.span></Text>
                        </HStack>
                        <HStack align="center" spacing={1.5}>
                            <Box w="8px" h="8px" bg="yellow.400" borderRadius="full" />
                            <Text fontSize="xs">Medium: <chakra.span fontWeight="bold" color="whiteAlpha.800">{analyticsData.difficulties.medium}</chakra.span></Text>
                        </HStack>
                        <HStack align="center" spacing={1.5}>
                            <Box w="8px" h="8px" bg="red.400" borderRadius="full" />
                            <Text fontSize="xs">Hard: <chakra.span fontWeight="bold" color="whiteAlpha.800">{analyticsData.difficulties.hard}</chakra.span></Text>
                        </HStack>
                    </HStack>
                </VStack>
            </SimpleGrid>
